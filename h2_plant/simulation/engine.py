...
class SimulationEngine:
    ...
    def __init__(
        ...
        self.flow_network = FlowNetwork(
            ...
            flow_tracker=self.monitoring.flow_tracker
        )
    ...
    def run(
        ...
        try:
            for step in range(start_step, end_step):
                ...
                # Update tracker time at start of step so FlowNetwork records with correct timestamp
                self.monitoring.flow_tracker.set_current_hour(hour)
    ...
    def _generate_results(self, start_hour: int, end_hour: int) -> Dict[str, Any]:
        ...
        metrics = self.monitoring.get_summary(history=self.get_dispatch_history())
        ...
        try:
            # Pass engine=self so dashboard can access dispatch history
            dashboard_data_file = self.monitoring.export_dashboard_data(engine=self)
