...
class CoolingManager(Component):
    ...
    def step(self, t: float) -> None:
        ...
        # ================================================================
        # Dynamic U-Value Calculation (Central Air Cooler Bank)
        # ================================================================
        # Geometry estimation for large central bank
        D_HYD = 0.02  # Hydraulic diameter for tube-side (m)
        A_FLOW_GLYCOL = self.dc_total_area_m2 * 0.005  # Pipe flow area approx
        A_FACE_AIR = self.dc_total_area_m2 / 40.0      # Air face area approx

        # Reynolds calculations
        Re_gly = numba_ops.calculate_reynolds_flux(
            self.glycol_flow_total_kg_s, A_FLOW_GLYCOL, D_HYD, DCC.MU_GLYCOL_PA_S
        )
        Re_air = numba_ops.calculate_reynolds_flux(
            self.dc_air_flow_kg_s, A_FACE_AIR, D_HYD, DCC.MU_AIR_PA_S
        )

        # Heat transfer coefficients
        # Glycol (inside tubes) - cooling mode
        Nu_gly = numba_ops.calculate_nusselt_dittus_boelter(Re_gly, DCC.PR_GLYCOL, False)
        h_gly = (Nu_gly * DCC.K_GLYCOL_W_M_K) / D_HYD

        # Air (crossflow over fins)
        Nu_air = numba_ops.calculate_nusselt_crossflow(Re_air, DCC.PR_AIR)
        h_air = (Nu_air * DCC.K_AIR_W_M_K) / D_HYD * DCC.ETA_FIN_DC

        # Calculate dynamic U-value with fouling (aluminum tube k ≈ 200 W/mK)
        K_TUBE_WALL = 200.0  # Aluminum thermal conductivity
        dynamic_u = numba_ops.calculate_dynamic_u_fouled(
            h_gly, h_air, D_HYD, D_HYD, K_TUBE_WALL, DCC.R_FOUL_GLYCOL, DCC.R_FOUL_AIR
        )

        # Use dynamic U if valid, otherwise fallback to design value
        if dynamic_u > 0:
            self.dc_u_value = dynamic_u
        # else: keep existing dc_u_value (initialized to 35.0)
