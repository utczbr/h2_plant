...
class DryCoolerIndirectConstants:
    ...
    # --- Detailed Geometry for Reynolds/Nusselt Calculations ---
    # TQC (Shell & Tube Interchanger - Process Gas Side)
    D_TUBE_IN_M: Final[float] = 0.018       # Inner tube diameter (Gas side)
    D_TUBE_OUT_M: Final[float] = 0.022      # Outer tube diameter (Glycol side)
    TUBE_LENGTH_M: Final[float] = 4.0       # Effective tube length per pass
    
    # DC (Finned Tube Air Cooler)
    D_TUBE_AIR_M: Final[float] = 0.020      # Tube outer diameter
    FIN_SPACING_M: Final[float] = 0.0025    # Fin spacing
    ETA_FIN_DC: Final[float] = 0.9          # Fin efficiency
    N_ROWS_DC: Final[float] = 4.0           # Tube bank depth

    # --- Fluid Transport Properties ---
    # Glycol (40% solution, approx values at ~30°C)
    RHO_GLYCOL_KG_M3: Final[float] = 1050.0
    K_GLYCOL_W_M_K: Final[float] = 0.45     # Thermal conductivity
    MU_GLYCOL_PA_S: Final[float] = 3.0e-3   # Dynamic viscosity
    PR_GLYCOL: Final[float] = 28.0          # Prandtl number
    
    # Air (Standard Conditions)
    MU_AIR_PA_S: Final[float] = 1.85e-5     # Dynamic viscosity
    K_AIR_W_M_K: Final[float] = 0.026       # Thermal conductivity
    PR_AIR: Final[float] = 0.71             # Prandtl number

    # --- Fouling Factors (m²·K/W) ---
    R_FOUL_GLYCOL: Final[float] = 0.0002
    R_FOUL_AIR: Final[float] = 0.0005
