# plots_modulos/plot_deoxo_perfil.py (CÓDIGO COMPLETO E CORRIGIDO)
...
def plot_deoxo_perfil(df_h2: pd.DataFrame, L_span: list, T_profile_C: list, X_O2: float, T_max_calc: float, deoxo_mode: str, L_deoxo: float, mostrar_grafico: bool = False):
    ...
    # 🛑 CORREÇÃO PRINCIPAL: Converter listas de entrada para arrays NumPy
    try:
        L_span = np.array(L_span, dtype=float)
        T_profile_C = np.array(T_profile_C, dtype=float)
    except Exception as e:
        print(f"AVISO: Falha na conversão de lista para array em plot_deoxo_perfil: {e}")
        return # Aborta a plotagem se a conversão falhar

    
    deoxo_df = df_h2[df_h2['Componente'] == 'Deoxo']

    # 🛑 CORREÇÃO DE LÓGICA: Usar .size após a conversão
    # Adicionando uma verificação robusta para evitar erro se L_span for [0.0]
    if not deoxo_df.empty and L_span.size > 0 and T_profile_C.size >= 1:
        ...
        for i in range(0, L_span.size, step):
            # 🛑 CORRIGINDO AMBIGUIDADE (Embora não seja o principal culpado, é boa prática)
            if X_profile[i] > 0:
