...
DP_PARTICLE_M = 0.003      # Diâmetro médio da partícula do adsorvente (m)
...
def calcular_trabalho_comp_purga(dot_m_purga_total, T_K, P_adsorcao_Pa, P_regeneracao_Pa):
    ...
    # 🛑 REMOVIDA A LÓGICA DE FALLBACK/ERRO DE DIVISÃO POR ZERO
...
def modelar_psa(m_dot_g_kg_s: float, P_in_bar: float, T_in_C: float, y_H2O_in: float, y_O2_in: float, N_vasos_int: int = N_VASOS) -> dict:
    ...
    # Cálculo da Densidade e Viscosidade (CoolProp)
    rho_mix_in = PropsSI('D', 'P', P_in_Pa, 'T', T_K, 'H2')
    mu_mix_in = PropsSI('V', 'P', P_in_Pa, 'T', T_K, 'H2') 
    
    # 🛑 REMOVIDO BLOCO DE AVISO DE FALLBACK
    ...
    results = {
        ...
        # 🛑 CORRIGIDO: Adicionado y_O2_out para evitar KeyError em process_execution.py
        "y_O2_out": y_O2_in, 
        ...
        # 💥 PSA não gera líquido acompanhante (é adsorvido/purgado)
        "m_dot_H2O_liq_accomp_out": 0.0,
        "Agua_Pura_Removida_H2O_kg_s": dot_m_H2O_in # Água removida (que vai para o pool)
    }
