...
# 🛑 NOVO: Importa P_PERDA_BAR, M_DOT_REF_H2 e M_DOT_REF_O2 do arquivo de constantes
try:
    from constants_and_config import P_PERDA_BAR, M_DOT_REF_H2, M_DOT_REF_O2 
except ImportError:
    # Fallback seguro caso as constantes não sejam encontradas
    P_PERDA_BAR = 0.05 
    M_DOT_REF_H2 = 5.0
    M_DOT_REF_O2 = 1.0
...
# REMOVIDAS daqui: M_DOT_REF_H2 = 5.0 e M_DOT_REF_O2 = 1.0
...
def simular_sistema_resfriamento(gas_fluido: str, m_dot_mix_kg_s: float, m_dot_H2O_liq_kg_s: float, P_in_bar: float, T_g_in_C: float, m_dot_a_op: float = None) -> dict:
    ...
    # Constantes M_DOT_REF_H2 e M_DOT_REF_O2 agora são importadas.
    if gas_fluido == 'H2':
...
# =================================================================
# === FUNÇÃO DE MODELAGEM (WRAPPER PARA COMPATIBILIDADE) ===
# =================================================================
...
def modelar_dry_cooler(gas_fluido: str, m_dot_mix_kg_s: float, m_dot_H2O_liq_kg_s: float, P_in_bar: float, T_g_in_C: float, m_dot_a_op: float = None) -> dict:
    """
    Função Wrapper para manter a compatibilidade com a chamada em process_execution.py.
    Chama o novo sistema de resfriamento Dry Cooler em duas etapas (TQC + DC).
    """
    
    # Chama a nova função de simulação que representa o Dry Cooler/TQC
    results = simular_sistema_resfriamento(
        gas_fluido, 
        m_dot_mix_kg_s, 
        m_dot_H2O_liq_kg_s, 
        P_in_bar, 
    ...
    # Adiciona uma chave de 'Agua_Condensada_kg_s' (0.0 no Dry Cooler) para compatibilidade
    if 'erro' not in results:
        results['Agua_Condensada_kg_s'] = 0.0
