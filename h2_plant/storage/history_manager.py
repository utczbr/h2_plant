...
class ChunkedHistoryManager:
    ...
    def buffers(self) -> Dict[str, np.ndarray]:
        """
        Direct access to current chunk arrays for high-performance writing.
        """
    ...
    def set_chunk_data(self, column: str, values: np.ndarray) -> None:
        """
        Efficiently set the entire data for a column in the current chunk.
        
        Args:
            column: Column name
            values: NumPy array of values (must match chunk_size)
        """
        if column not in self._current_chunk:
            self.register_column(column)
            if column not in self._current_chunk:
        ...
        # Direct array assignment (fast)
        # Ensure length match to avoid broadcasting errors
        n = min(len(values), self.chunk_size)
        self._current_chunk[column][:n] = values[:n]
    ...
    def flush(self) -> Optional[Path]:
        ...
        # Write to Parquet
        self.chunks_dir.mkdir(parents=True, exist_ok=True)  # CRITICAL: Ensure dir exists even if deleted during run
