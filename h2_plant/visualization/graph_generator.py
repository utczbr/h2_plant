    ...
    def export(self, graph_id: str, output_path: Union[str, Path], 
        ...
        try:
            if format == 'html':
                ...
                # Ensure editable config is set
                plot_config = kwargs.pop('config', {'displayModeBar': True, 'responsive': True})
                if isinstance(plot_config, dict):
                    plot_config['editable'] = True
                
                fig.write_html(str(output_path), include_plotlyjs='cdn', config=plot_config, **kwargs)
