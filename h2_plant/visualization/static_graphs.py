...
def create_dry_cooler_figure(df: pd.DataFrame, dpi: int = DPI_FAST) -> Figure:
    ...
    for comp_id, data in heat_rejected.items():
        ax1.plot(x_ds, downsample_for_plot(data), 
    ...
    for comp_id, data in outlet_temp.items():
        # Convert K to °C if needed
        data_c = data - 273.15 if data.mean() > KELVIN_DETECTION_THRESHOLD else data
        ax2.plot(x_ds, downsample_for_plot(data_c), 
                 label=comp_id, linewidth=1.5, color='teal')
    ax2.set_ylabel('Outlet Temperature (°C)')
    ax2.set_title('Outlet Temperature')
    ax2.grid(True, linestyle='--', alpha=0.5)
    if outlet_temp:
        ax2.legend(loc='upper right', fontsize=8)

    # Panel 3: U-Value (Dynamic Heat Transfer)
    u_values = _find_component_columns(df, 'DryCooler', 'tqc_u_value')
    # Also define generic search for U-values
    if not u_values:
        u_values = _find_component_columns(df, 'DryCooler', 'u_value')
        
    for comp_id, data in u_values.items():
        ax3.plot(x_ds, downsample_for_plot(data), 
                 label=comp_id, linewidth=1.5, color='purple')
    ax3.set_ylabel('U-Value (W/m²K)')
    ax3.set_xlabel('Time (Minutes)')
    ax3.set_title('Dynamic U-Value')
    ax3.grid(True, linestyle='--', alpha=0.5)
    
    # Panel 4: Effectiveness
    eff_values = _find_component_columns(df, 'DryCooler', 'tqc_effectiveness')
    if not eff_values:
        eff_values = _find_component_columns(df, 'DryCooler', 'effectiveness')
        
    for comp_id, data in eff_values.items():
        ax4.plot(x_ds, downsample_for_plot(data * 100), 
                 label=comp_id, linewidth=1.5, color='green')
    ax4.set_ylabel('Effectiveness (%)')
    ax4.set_xlabel('Time (Minutes)')
    ax4.set_title('Heat Exchanger Effectiveness')
    ax4.set_ylim(0, 100)
