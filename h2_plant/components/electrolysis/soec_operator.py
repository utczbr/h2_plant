...
class SOECOperator(Component):
    ...
    def _initialize_state(self) -> None:
        ...
        self.cycle_counts = np.zeros(self.num_modules, dtype=np.int32)
        ...
        # Initialize step tracking variables to valid defaults (prevents crash on early get_state)
        self.last_step_steam_input_kg = 0.0
        self.last_water_output_kg = 0.0
        self.last_step_h2_kg = 0.0
    ...
    def step(self, t: float = 0.0) -> Tuple[float, float, float]:
        ...
        for i in range(self.num_modules):
            ...
            eff_i = float(self._interpolate(effective_year, DEG_YEARS, DEG_EFFICIENCY_KWH_KG))
            self.module_efficiencies[i] = eff_i
            deg_factors[i] = eff_i / self.bol_efficiency_kwh_kg
        
        # Keep legacy scalar for backward compatibility (use mean of active modules)
        active_mask = self.real_states > 1
        if np.any(active_mask):
            self.current_efficiency_kwh_kg = float(np.mean(self.module_efficiencies[active_mask]))
        else:
        ...
        if abs(new_capacity_factor - self.current_capacity_factor) > 1e-6:
            ...
            active_lim_mask = self.real_states != 0
            self.real_limits[active_lim_mask] = self.uniform_module_max_limit
        ...
        # Calculate H2 production using vectorized per-module degradation factors
    ...
    def restore_state(self, state: Dict[str, Any]) -> None:
        ...
        This method correctly handles the deserialization of NumPy arrays that
        were saved as lists in JSON checkpoints. It also maps state keys
        (e.g., 'module_powers') to their corresponding internal attribute names
        (e.g., 'real_powers').
        ...
        # Handle NumPy arrays that were serialized as lists
        if 'module_powers' in state:
            self.real_powers = np.array(state['module_powers'], dtype=np.float64)
        
        if 'module_states' in state:
            self.real_states = np.array(state['module_states'], dtype=np.int32)
            # Also update previous_real_states to avoid false cycle counts on resume
            self.previous_real_states = np.copy(self.real_states)
        ...
        if 'cycle_counts' in state:
            self.cycle_counts = np.array(state['cycle_counts'], dtype=np.int32)
            
        # Restore scalar values that might have aliases or special handling
        if 'total_power_mw' in state:
            self.previous_total_power = state['total_power_mw']
            
        if 'total_h2_kg' in state:
            self.total_h2_produced = state['total_h2_kg']
        
        # It's good practice to re-calculate dependent state if possible,
        # but in this case, direct restoration of the main arrays is sufficient.
        # For example, re-deriving virtual_map based on restored real_states.
        active_indices = np.where(self.real_states != 0)[0]
        self.virtual_map = active_indices.astype(np.int32)

        # Log that state was restored to aid debugging
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"SOECOperator state successfully restored for component {self.component_id}.")
