...
class ElectricBoiler(Component):
    ...
    def __init__(self, config: Dict[str, Any] = None, **kwargs) -> None:
        ...
        # Thermostat mode (accept both 'target_temp_c' and 'target_outlet_temp_c')
        self.target_temp_k = None
        target_temp_c = config.get('target_temp_c') or config.get('target_outlet_temp_c')
    ...
    def initialize(self, dt: float, registry: 'ComponentRegistry') -> None:
        ...
        self.dt_hours = dt
        self.lut = registry.get('lut_manager') if registry and hasattr(registry, 'has') and registry.has('lut_manager') else None
    ...
    def step(self, t: float) -> None:
        ...
        # --- Merge Entrained Liquid (Standardize Total Mass) ---
        # Checks for entrained liquid in 'extra' and merges it into the bulk stream
        if inflow and inflow.extra and 'm_dot_H2O_liq_accomp_kg_s' in inflow.extra:
            entrained_kg_s = inflow.extra.get('m_dot_H2O_liq_accomp_kg_s', 0.0)
            entrained_kg_h = entrained_kg_s * 3600.0
            
            if entrained_kg_h > 0:
                # Calculate new total mass
                m_old = inflow.mass_flow_kg_h
                m_new = m_old + entrained_kg_h
                
                # Update composition
                comp = inflow.composition.copy()
                
                # Existing masses
                m_h2o_liq_old = comp.get('H2O_liq', 0.0) * m_old
                m_h2o_liq_new = m_h2o_liq_old + entrained_kg_h
                
                # Recalculate fractions
                comp['H2O_liq'] = m_h2o_liq_new / m_new
                for s in comp:
                    if s != 'H2O_liq':
                        comp[s] = (comp[s] * m_old) / m_new
                
                # Clean extra (remove merged liquid)
                new_extra = inflow.extra.copy()
                del new_extra['m_dot_H2O_liq_accomp_kg_s']

                # Create new merged input stream
                inflow = Stream(
                    mass_flow_kg_h=m_new,
                    temperature_k=inflow.temperature_k,
                    pressure_pa=inflow.pressure_pa,
                    composition=comp,
                    phase=inflow.phase,
                    extra=new_extra
                )
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            **super().get_state(),
            ...
            # Explicit Mass Flow metrics (was missing)
            "outlet_mass_flow_kg_h": self._output_stream.mass_flow_kg_h if self._output_stream else 0.0,
            "outlet_total_mass_flow_kg_h": self._output_stream.mass_flow_kg_h if self._output_stream else 0.0
        }
