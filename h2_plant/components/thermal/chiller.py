...
class Chiller(Component):
    ...
    def step(self, t: float) -> None:
        ...
        # MASS INTEGRITY CHECK
        # Verify that Mass_Out ≈ Mass_In + Extra_In
        expected_mass_out = self.inlet_stream.mass_flow_kg_h + (
            self.inlet_stream.extra.get('m_dot_H2O_liq_accomp_kg_s', 0.0) * 3600.0 
            if hasattr(self.inlet_stream, 'extra') and self.inlet_stream.extra else 0.0
        )
        if abs(outlet_mass_flow - expected_mass_out) > 1e-4:
            self.logger.warning(
                f"Chiller {self.component_id} MASS BALANCE VIOLATION: "
                f"Expected {expected_mass_out:.4f}, Got {outlet_mass_flow:.4f}"
            )
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            **super().get_state(),
            ...
            # Explicit Mass Flow metrics (Standardized)
            'outlet_mass_flow_kg_h': self.outlet_stream.mass_flow_kg_h if getattr(self, 'outlet_stream', None) else 0.0,
            'outlet_total_mass_flow_kg_h': self.outlet_stream.mass_flow_kg_h if getattr(self, 'outlet_stream', None) else 0.0,
            'outlet_gas_mass_flow_kg_h': self.outlet_stream.mass_flow_kg_h if getattr(self, 'outlet_stream', None) else 0.0, # Chiller merges liquid
            'outlet_entrained_mass_kg_h': 0.0,
