...
class DryCooler(Component):
    ...
    *   $T_{glycol, cold}$: Temperature leaving DC, entering TQC (re-circulated).
    ...
    def __init__(self, component_id: str = "dry_cooler", use_central_utility: bool = True, **kwargs) -> None:
        ...
        self.target_outlet_temp_c = kwargs.get(
            'target_outlet_temp_c', 
            kwargs.get('target_temp_c', 
            kwargs.get('target_out_temp_c', None))  # Support all naming variations
        )
        ...
        # Geometry cache for dynamic heat transfer calculations
        self._tqc_n_tubes = 0.0
        self._tqc_flow_area_gas = 0.0
        self._tqc_flow_area_glycol = 0.0
    ...
    def _configure_geometry(self, stream: Stream) -> None:
        ...
        # Geometry cache for dynamic heat transfer (compute once)
    ...
    def _update_geometry_cache(self) -> None:
        """
        Estimate flow areas from heat transfer geometry for Reynolds calculations.
        
        Uses the relationship: Area = N_tubes × π × D × L to estimate tube count,
        then computes flow cross-sectional areas.
        """
        import numpy as np
        
        if self.tqc_area_m2 > 0:
            # TQC: Estimate number of tubes from heat transfer area
            # A = N × π × D_in × L (based on inner surface)
            self._tqc_n_tubes = self.tqc_area_m2 / (
                np.pi * DCC.D_TUBE_IN_M * DCC.TUBE_LENGTH_M
            )
            
            # Gas side flow area (tube-side): N × π × (D_in/2)²
            self._tqc_flow_area_gas = self._tqc_n_tubes * np.pi * (DCC.D_TUBE_IN_M / 2.0) ** 2
            
            # Shell side flow area (glycol): Approximation ~1.5× tube area
            # This is a design heuristic for shell-and-tube exchangers
            self._tqc_flow_area_glycol = self._tqc_flow_area_gas * 1.5
        else:
            self._tqc_n_tubes = 1.0
            self._tqc_flow_area_gas = 1e-4  # Fallback
            self._tqc_flow_area_glycol = 1.5e-4
        ...
        **Layer 1 Contract**:
        ...
        return {
            **super().get_state(),
            ...
            'heat_rejected_kw': self.tqc_duty_kw,  # Alias for engine_dispatch (shows Process Load)
            ...
            'tqc_u_value': self.tqc_u_value,
            'dc_u_value': self.dc_u_value,
            'latent_heat_kw': self.latent_heat_kw,
            'cooling_load_kw': self.tqc_duty_kw,  # Alias for visualization (matches Chiller pattern)
            'heating_load_kw': 0.0, # Interface consistency
            'outlet_entrained_mass_kg_h': 0.0, # Dry Cooler merges everything to bulk
            'outlet_mass_flow_kg_h': self.outlet_stream.mass_flow_kg_h if self.outlet_stream else 0.0, # Bulk (Already Total)
            'outlet_total_mass_flow_kg_h': self.outlet_stream.mass_flow_kg_h if self.outlet_stream else 0.0, # Standardized
            'outlet_gas_mass_flow_kg_h': self.outlet_stream.mass_flow_kg_h if self.outlet_stream else 0.0, # Bulk (Process Path)
