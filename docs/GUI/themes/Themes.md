...
### 1. `theme_manager.py` (Final Version)
This file replaces or updates your existing theme file. It now includes `QPalette` handling for a deeper dark mode application and explicit grid management.
...
### 2. Integration Steps (Update `main_window.py`)

Update your `PlantEditorWindow` to use this manager.
...
3.  **Add Theme Switcher Menu:**
    Inside `setup_advanced_menu(self)`:
    ```python
    # View Menu
    ...
    theme_menu.addAction("Dark (VS Code Pro)", 
        lambda: ThemeManager.apply_theme(self, QApplication.instance(), "dark"))
        
    theme_menu.addAction("Light (System)", 
        lambda: ThemeManager.apply_theme(self, QApplication.instance(), "light"))
    ```
