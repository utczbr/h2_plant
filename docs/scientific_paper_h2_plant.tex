...
To achieve a 25-year lifecycle simulation at \SI{1}{\minute} resolution, the computational engine must execute over 13 million timesteps. If each timestep requires dozens of thermodynamic property calls, the cumulative latency of standard libraries renders the simulation intractable. To resolve this, the architecture employs a two-tier optimization strategy: \textbf{Thermodynamic Caching} (Look-Up Tables) and \textbf{Just-In-Time Compilation} (Numba JIT).

\subsection{Look-Up Table (LUT) Architecture}

Thermodynamic state calculation traditionally requires iterative solutions to complex Equations of State (EOS). The \texttt{LUTManager} circumvents this by implementing a ``Generate Once, Cache Forever'' strategy.

\subsubsection{The Topographical Map Analogy}

To explain the necessity of this approach, consider a hiker navigating difficult terrain (the thermodynamic state space):

\begin{itemize}
    \item \textbf{CoolProp Approach (Calculation)}: Without a map, every step requires solving complex equations to determine altitude. Accurate, but slow ($\sim$\SI{1000}{\micro\second} per call).
    \item \textbf{LUT Approach (Interpolation)}: The simulation pre-prints a high-resolution topographic map. Property retrieval becomes a simple lookup on the map---orders of magnitude faster ($\sim$\SI{1}{\micro\second}).
\end{itemize}

\subsubsection{Grid Discretization Strategy}

The ``map'' is constructed as a 2D grid of Pressure ($P$) and Temperature ($T$). However, a uniform checkerboard grid is inefficient because gases behave differently across pressure regimes.
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Axis} & \textbf{Range} & \textbf{Points} & \textbf{Spacing Type} \\
\midrule
Pressure & \SI{0.05}{\bar}--\SI{1000}{\bar} & 2000 & Logarithmic (\texttt{geomspace}) \\
Temperature & \SI{273}{\kelvin}--\SI{1200}{\kelvin} & 2000 & Linear (\texttt{linspace}) \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Why Logarithmic Pressure Spacing?}
At low pressures (e.g., \SI{1}{\bar}), a \SI{0.1}{\bar} change significantly impacts gas density; at high pressures (e.g., \SI{700}{\bar}), the same change is negligible. The \texttt{np.geomspace} function concentrates thousands of points in the low-pressure region where the ``terrain'' is steep, while spreading points in the high-pressure region where physics are stable. This maintains \textit{uniform relative error} across four orders of magnitude.

\subsubsection{Bilinear Interpolation}

When the simulation requests a value between grid points, the \texttt{LUTManager} performs bilinear interpolation---a weighted average of the four surrounding neighbors---allowing the discrete grid to function as a continuous surface with $O(h^2)$ error scaling.

\paragraph{Properties Cached}: Density ($\rho$), Enthalpy ($H$), Entropy ($S$), Heat Capacity ($C_p$), Compressibility ($Z$).

\paragraph{Fluids}: H$_2$, O$_2$, H$_2$O, CO$_2$, CH$_4$, N$_2$.
...
While Python offers high-level flexibility for architecture, its interpreted nature introduces significant overhead for mathematical loops (the ``hot paths''). The \texttt{numba\_ops} module mitigates this via Just-In-Time (JIT) compilation.

\subsubsection{Mitigating Interpreter Overhead}

In standard Python, every arithmetic operation involves type-checking and object boxing/unboxing. The \texttt{@njit} decorator instructs the LLVM compiler to translate Python functions into optimized machine code at runtime, bypassing the Global Interpreter Lock (GIL) and allowing raw floating-point operations at speeds comparable to C or Fortran.

\subsubsection{Algorithmic Optimization: The Single-Condensable Flash}

Standard Vapor-Liquid Equilibrium (VLE) calculations require iterative solvers (Newton-Raphson) to find the vapor fraction $\beta$. However, for the specific case where \textit{only water condenses}---a valid assumption for H$_2$/H$_2$O mixtures---the problem simplifies to an analytical solution:

\begin{lstlisting}[caption={JIT-Compiled Analytical Flash Solver}]
@njit
def solve_rachford_rice_single_condensable(
    z_condensable: float,
    K_value: float
) -> float:
    """O(1) complexity: bypasses Newton-Raphson iteration."""
    # Thermodynamic bounds check
    if K_value >= 1.0:       # Superheated (all vapor)
        return 1.0
    if z_condensable <= K_value:  # Undersaturated
        return 1.0
    # Analytical solution for vapor fraction
...
By combining \textbf{Algorithmic Reduction} (replacing iterative solver with $O(1)$ algebraic solution) with \textbf{Machine Code Generation} (Numba JIT), this kernel reduces computational cost from $\sim$\SI{500}{\micro\second} (iterative Python) to $<$\SI{50}{\nano\second} (JIT analytical)---a \textbf{10,000$\times$ speedup}. This is critical for flash calculations executing at every timestep across multiple separation units.
